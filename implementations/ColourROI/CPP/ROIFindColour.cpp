...
vector<int> ROIFindColour(cv::Mat im)
{
    ...
    */
    ...
    // std::cout << "about to create samples" << std::endl;
    for(int y = im.size[0]*2/3; y < im.size[0]; y += 5)
    {
    ...
    // std::cout << "finding green" << std::endl;
    ...
    for(int c = 0; c < samples.size(); ++c)
    {
        ...
        {
            // std::cout << "sample size: " << samples[c].size() << std::endl;
...
static boost::python::object vectorWrapper (cv::Mat baseIm) {
    ...
    // std::cout << "starting colour ROI finder" << std::endl;
